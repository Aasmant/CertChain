...
contract cert{
    ...
    function isInstituteExists(address instituteAddress) public view returns (bool) {
        institute memory inst = institutes[instituteAddress];
        return (bytes(inst.name).length > 0) && !inst.del;
    }
    ...
    function issueCertificate(address certad,address instad,address studad,string memory f_name,string memory l_name,string memory _course,int dur) public
    {
        if(institutes[instad].del == true)
            return;
        if(institutes[msg.sender].del == true)
        ...
        string memory instName = institutes[instad].name;
        
        certificates[certad] = certificate(f_name, l_name, studad, instad, instName, _course, dur, false);
    ...
    function getStudentName(address studad) public view returns (string memory) {
        return students[studad];
    }
